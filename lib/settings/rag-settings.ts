...
export async function loadRAGSettings(): Promise<RAGSettings> {
  try {
    ...
    console.log('Using default RAG settings')
    return DEFAULT_SETTINGS
  }
}

export function getEffectiveModel(settings: RAGSettings): string {
  return settings.customModel || settings.model
}

export async function saveRAGSettings(settings: RAGSettings): Promise<void> {
